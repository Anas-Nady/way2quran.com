...
export async function generateMetadata({ params: { locale } }) {
  const t = await getTranslations({ locale, namespace: "HolyQuranPage" });
  const pageUrl = `full-holy-quran`;
  const title = t("holyQuran");

  return shareMetadata({ title, locale, pageUrl });
}

export default async function FullHolyQuranPage({
  params: { locale },
  searchParams,
}) {
  const t = await getTranslations({ locale, namespace: "HolyQuranPage" });
